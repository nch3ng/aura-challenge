...
describe("basic tests", () => {
  ...
  describe('with query parameters', () => {
    it('should look up zipcodes with full city name', async () => {
      ...
      await expect(handler(event)).resolves.toHaveProperty('zips', full_city_search_result);  
    }); 
    
    it('should look up zipcodes with partial city name', async () => {
      const event = buildQueryParams('city', 'Test City1');

      await expect(handler(event)).resolves.toHaveProperty('zips', partial_city_search_result);  
    });

    it('should look up zipcodes with full zipcode', async () => {
      const event = buildQueryParams('zipcode', '01013');

      await expect(handler(event)).resolves.toHaveProperty('zips', full_zip_search_result);  
    });
    it('should look up zipcodes with partial zipcode', async () => {
      const event = buildQueryParams('zipcode', '0101');

      await expect(handler(event)).resolves.toHaveProperty('zips', partial_zip_search_result);  
    });

    it('should look up the closest zipcode with the coordinate', async () => {
      const event = buildQueryParams('coordinate', [42.45, -72.90]);
    ...
    it('should return error if coordinate out of boundary', async () => {
      ...
      await expect(handler(event)).resolves.toHaveProperty('error', 'Current verion only support the search within the boundary');  
    });
  ...
  describe('with request body', () => {
    ...
    it('should look up the closest zipcode with the coordinate', async () => {
      const event = buildRequestBody('coordinate', [42.45, -72.90]);
